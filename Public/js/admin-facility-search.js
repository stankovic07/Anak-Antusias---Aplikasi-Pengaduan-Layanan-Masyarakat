...
function renderTable(facilities) {
  ...
  if (!facilities.length) {
    tbody.innerHTML = `<tr><td colspan="6" class="text-center py-12 text-gray-400"><i class="fa-solid fa-building-circle-xmark text-3xl mb-2 block"></i>Tidak ada fasilitas yang ditemukan</td></tr>`;
    return;
  }

  tbody.innerHTML = facilities.map(f => {
    const imgHtml = f.image_path
      ? `<img src="${f.image_path}" alt="${f.name}" class="facility-img" onerror="this.parentElement.innerHTML='<div class=\"facility-img bg-gray-100 flex items-center justify-center text-gray-400\"><i class=\"fa-solid fa-image\"></i></div>'" />`
